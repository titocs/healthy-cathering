...
$kategori = query("SELECT * FROM $jenis");
...
<body class="bg-[#F5F5F8] p-5 relative">
    <div class="flex items-center gap-3 mb-5">
        <button onclick="history.back()">
            <img class="w-3" src="/icon/angle-left-solid.svg" alt="">
        </button>
        <h1 class="text-3xl font-semibold"><?php echo $jenis ?></h1>
    </div>

    <div class="wrapper grid grid-cols-3 gap-6 gap-y-4 mb-20">
        <?php foreach($kategori as $ktg): ?>
        <div class="">
            <div class="h-24">
                <img class="w-full h-full object-cover rounded-md drop-shadow-lg" src="/img/<?php echo $ktg['gambar']; ?>" alt="">
            </div>
            <p class="text-sm font-semibold"><?php echo $ktg['kategori']; ?></p>
        </div>
        <?php endforeach; ?>
    </div>
    ...
    <div class="bg-main-color fixed bottom-0 left-0 right-0 p-4 flex justify-between">
        <a class="bg-white p-3 rounded-full" href="home.php">
